...
function Job_details() {
  ...
  useEffect(() => {
    fetch(`http://127.0.0.5:3000/api/jobs/job/${id}`).then(res=>res.json()).then(data=>setjob(data))
  }, [])
  return (
    <div className="bg-white rounded-lg mt-20 h-screen shadow-md p-6">
      ...
      <div className="flex mb-4">
        ...
        <p className="ml-2">{job.experienceLevel}</p>
      </div>
      <div className="grid grid-cols-4 gap-4">
        <div className="flex items-center">
          ...
          <p className="ml-2">{job.employmentType}</p>
        </div>
        <div className="flex items-center">
        ...
        <div className="flex items-center">
          ...
          <p className="ml-2">₹ {job.minPrice}-{job.maxPrice} /month</p>
        </div>
        <div className="flex items-center">
          
          <p className="ml-2">Posting Date: {job.postingDate}</p>
        </div>
      </div>
